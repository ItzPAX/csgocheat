...
{
	FARPROC result = pGetProcAddress(hMod, name);
	std::cout << "[VACFuck] Steam is trying to import: " << name << std::endl;
...
void patch_vac(void* DllImageBase)
{
	...
	PVOID invalid_je = PatternScan(DllImageBase, "74 19 56 8B CB E8 ? ? ? ? C7 46 10 ? ? ? ? 32 C0");
	DWORD oldProt;
	VirtualProtect(invalid_je, 1, PAGE_EXECUTE_READWRITE, &oldProt);
	*(BYTE*)(invalid_je) = 0x72;
	
	std::cout << "[VACFuck] Patched VAC-Loader!" << std::endl;
	
	/*PVOID patch_mm = PatternScan(DllImageBase, "74 47 6A 01 6A 00 FF 76 18");
	BYTE patch[1] = { 0xEB };
	DWORD oldProt;
	VirtualProtect(patch_mm, 1, PAGE_EXECUTE_READWRITE, &oldProt);
	*(BYTE*)(patch_mm) = 0xEB;*/
}
...
bool InitVACHooks(void* DllImageBase) {
	...
	//do it last to not cause problems
